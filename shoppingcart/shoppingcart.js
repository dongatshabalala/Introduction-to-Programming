...
function total_selection() {
    ...
    // CALCULATING THE TOTAL AMOUNT OF ITEMS PURCHASED

    let total = 0;
    for (let i = 0; i < totals.length; i++) {
        total += totals[i];
    }
    console.log(`Total amount of items purchased: ${total} units of currency.`);

    // DISPLAYING THE TOTAL AMOUNT ON THE PAGE

    let totalAmountElement = document.getElementById("totalAmount");
    totalAmountElement.innerHTML = `Total amount: ${total} units of currency.`;
}
...
function display_all() {
    ...
    myTable += "<td><button onclick='total_selection()'>Check Out</button></td>";
